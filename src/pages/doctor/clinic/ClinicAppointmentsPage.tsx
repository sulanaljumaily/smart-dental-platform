  ...
  const handleUpdateAptStatusQuick = async (apt: Appointment, newStatus: any) => {
    try {
      ...
      if (newStatus === 'cancelled' || newStatus === 'noshow') {
        toast('💡 هل ترغب في تعديل موعد المريض لإعادة جدولته؟', {
          action: {
            label: 'تعديل الموعد الآن 🗓️',
            onClick: () => {
              setSelectedAptForDetails(null);
              setEditingAppointment(updatedApt);
              setIsModalOpen(true);
            }
          },
          duration: 8000
                      ...
                      <div className="flex items-center justify-between gap-4 w-full">
                        <h4 className="font-bold text-gray-900 text-lg flex items-center gap-2 min-w-0 truncate">
                          ...
                          {(() => {
                            ...
                            const isPast = aptDateOnly < todayStr;
                            const isPendingAction = isPast && (apt.status === 'scheduled' || apt.status === 'confirmed');
                            return isPendingAction ? (
                              <span className="px-2 py-0.5 rounded-lg text-[10px] font-black text-amber-700 bg-amber-50 border border-amber-200 animate-pulse flex items-center gap-1 shadow-sm flex-shrink-0">
                                <AlertCircle className="w-3 h-3 text-amber-600 animate-bounce" />
                                متأخر ⚠️
                              </span>
                            ) : null;
                          })()}
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      ...
                      {(() => {
                        ...
                        const isPendingAction = isPast && (apt.status === 'scheduled' || apt.status === 'confirmed');
                        return isPendingAction ? (
                          <span className="px-2.5 py-1 rounded-full text-xs font-black text-amber-700 bg-amber-50 border border-amber-200 animate-pulse flex items-center gap-1 shadow-sm">
                            <AlertCircle className="w-3.5 h-3.5 text-amber-600 animate-bounce" />
                            متأخر ويتطلب إجراء ⚠️
                          </span>
                        ) : null;
                      })()}
                    </div>
            ...
            <div className="p-6 space-y-6 overflow-y-auto flex-1 text-right" dir="rtl">
              ...
              <div className="bg-gray-50 p-5 rounded-2xl border border-gray-200/60 space-y-3">
                ...
                {(() => {
                  ...
                  const aptDateOnly = selectedAptForDetails.date.includes('T') ? selectedAptForDetails.date.split('T')[0] : selectedAptForDetails.date;
                  const todayStr = new Date().toLocaleDateString('en-CA');
                  const isPast = aptDateOnly < todayStr;
                  const isPendingAction = isPast && (status === 'scheduled' || status === 'confirmed');
                  ...
                  if (isPendingAction) {
                    diagnosticTitle = "⚠️ موعد متأخر يتطلب تحديث الحالة";
                    diagnosticDesc = "هذا الموعد قد فات تاريخه المجدد دون تحديد حالته النهائية (مكتمل، لم يحضر، أو ملغي). يرجى تحديث حالة الموعد الفورية بالأسفل لتنظيم سجلات العيادة.";
              ...
              {(() => {
                ...
                const statusOptions = isPast
                  ? allStatusOptions.filter(opt => ['noshow', 'cancelled', 'completed'].includes(opt.status))
                  : allStatusOptions;

                return (
                  <div className="space-y-3">
                    <label className="block text-sm font-bold text-gray-700">تحديث حالة الموعد الفورية</label>
                    <div className={`grid gap-2 ${isPast ? 'grid-cols-3 sm:grid-cols-3' : 'grid-cols-2 sm:grid-cols-5'}`}>
              ...
              {/* Reschedule CTA for Cancelled or No-Show appointments */}
              {(selectedAptForDetails.status === 'cancelled' || selectedAptForDetails.status === 'noshow') && (
                <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 animate-in fade-in slide-in-from-bottom-2">
                  <div className="text-right">
                    <h5 className="font-bold text-sm text-amber-900">💡 هل ترغب في إعادة جدولة الموعد؟</h5>
                    <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                      هذا الموعد {selectedAptForDetails.status === 'cancelled' ? 'ملغي' : 'لم يحضر فيه المريض'}. يمكنك تعديل التاريخ والوقت فوراً لإعادة جدولته.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      const aptToEdit = selectedAptForDetails;
                      setSelectedAptForDetails(null); // Close diagnostics modal
                      setEditingAppointment(aptToEdit); // Load appointment into edit state
                      setIsModalOpen(true); // Open add/edit modal!
                    }}
                    className="w-full sm:w-auto px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-xl text-xs shadow-md shadow-amber-600/10 flex items-center justify-center gap-1.5 transition-all active:scale-95 flex-shrink-0"
                  >
                    <Calendar className="w-3.5 h-3.5" />
                    تعديل وقت وتاريخ الموعد 🗓️
                  </button>
                </div>
              )}
