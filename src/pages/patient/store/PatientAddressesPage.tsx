...
import { MapPin, Plus, Trash2, Edit3, CheckCircle, Navigation, Building, Stethoscope } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {/* Info Request Card */}
        {!showForm && (
          <div className="bg-gradient-to-br from-teal-50 to-cyan-50 rounded-3xl p-6 border border-teal-100 flex items-center justify-between mb-8">
            <div>
              <h3 className="font-bold text-teal-900 mb-1">إضافة عنوان جديد</h3>
              <p className="text-sm text-teal-700/80">أضف عنوان التوصيل لاستقبال طلباتك بسهولة.</p>
            </div>
            <Button onClick={() => setShowForm(true)} className="bg-teal-600 hover:bg-teal-700 shadow-lg shadow-teal-200 text-white rounded-xl px-4 py-2 flex items-center gap-2">
          ...
          <Card className="p-6 mb-8 border-2 border-slate-200 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              ...
              <div className="space-y-2">
                ...
                <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-teal-200 outline-none" id="addr-gov">
                  {['بغداد','أربيل','البصرة','النجف','نينوى','ذي قار','كركوك','الأنبار','ديالى','المثنى','القادسية','ميسان','واسط','صلاح الدين','دهوك','السليمانية','بابل','كربلاء'].map(g => (
              ...
              <div className="space-y-2">
                ...
                <input type="text" className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 focus:ring-2 focus:ring-teal-200 outline-none" placeholder="مثال: المنصور" id="addr-city" />
              </div>
              <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {addresses.map(address => (
            <div
              key={address.id}
              className={`p-6 rounded-3xl transition-all duration-300 relative group ${address.isDefault ? 'bg-white shadow-lg border-2 border-teal-500' : 'bg-white shadow-sm border border-slate-100 hover:shadow-md'}`}
            >
              {address.isDefault && (
                <div className="absolute top-4 left-4 text-white bg-teal-500 flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                  <CheckCircle className="w-3 h-3" /> مفعل
                </div>
              )}
              <div className="mb-4">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 transition-colors ${address.type === 'clinic' ? 'bg-blue-50 text-blue-500' : address.type === 'lab' ? 'bg-purple-50 text-purple-500' : 'bg-slate-50 text-slate-400 group-hover:bg-teal-50 group-hover:text-teal-500'}`}>
                  {address.type === 'clinic' ? <Stethoscope className="w-5 h-5" /> : address.type === 'lab' ? <Building className="w-5 h-5" /> : <MapPin className="w-5 h-5" />}
                </div>
                <h3 className="font-bold text-lg text-slate-900">{address.name}</h3>
                <p className="text-slate-500 text-sm mt-1 leading-relaxed">{address.city}، {address.area}<br />{address.street}</p>
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-slate-50">
                ...
                {address.type === 'custom' && (
                  <button onClick={() => deleteAddress(address.id)} className="flex-1 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-lg transition-colors flex items-center justify-center gap-1">
